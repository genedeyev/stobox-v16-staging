...
(function(){
  ...
  var groups = Array.prototype.slice.call(
    document.querySelectorAll('.reveal, .rgroup, .wipe, .reveal-rule, .hero-seq'));
  var showAll = function(){
    groups.forEach(function(el){ el.classList.add('in'); });
    var h = document.querySelector('.hero-seq'); if (h) h.classList.add('in');
  };

  // the hero is already in view: it arrives on load, not on scroll
  ...
  // the header takes its rule only after you leave the top
  var head = document.querySelector('.site-head');
  if (head) {
