...
(function(){
  ...
  var sets = Array.prototype.slice.call(document.querySelectorAll('.gset'));
  ...
  function apply(){
    ...
    terms.forEach(function(t){
      var on = !v || t.getAttribute('data-k').indexOf(v) > -1;
      t.hidden = !on;
      if (on) n += 1;
    });
    sets.forEach(function(s){
      s.hidden = !s.querySelector('.term:not([hidden])');
    });
    count.textContent = n + ' of ' + total;
    empty.hidden = n > 0;
    live.textContent = n === total ? '' : (n + ' terms match');
  }
